...
This would be very secure as we shouldn't have a seed... However, we're provided with a `` file that showcases the workflow.
http://a668910b5514e.whiterabbit.htb/gophish/gophish_to_phishing_score_database.
...
```
 ...
 The text has been hinting at an `SQLi` attack, additionally several `SQL` queries can be found in the `` file which we grabbed earlier, I don't know about you but it sure as hell smells to me like an `SQLi` attack is in order.

## SQLi Attack

Note the following from the `` file:
...
Looking at the `` we can determine that the character used for the query is `"` and not the usual `'`.

```
{
"query": "SELECT * FROM victims where email = \"{{ $.body.email }}\" LIMIT 1"
}
```

...
```
curl the response link and we get:
```bash
...
Looking around we find the database:
...
>[!note] I had an LLM look at the code and adapt it to become easier to understand, the following is the result. Furthermore, if I had found the binary quicker I wouldn't have to do that trial and error.
